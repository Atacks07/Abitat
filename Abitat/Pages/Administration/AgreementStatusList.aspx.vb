...
Namespace Pages.Administration
    ...
    Public Class AgreementStatusList
        Inherits AuthenticatedPage

        Private ReadOnly _agreementStatusService As New AgreementStatusService()
        ...
        Protected Overrides ReadOnly Property RequiredPermission As String
            Get
                Return "AGREEMENTS_STATUSES_VIEW"
            End Get
        End Property
        ...
        Private Sub BindGrid()
            ...
            gvAgreementsStatuses.DataSource = _agreementStatusService.GetAll(search, statusId)
            gvAgreementsStatuses.DataBind()
        End Sub
        ...
        Protected Sub btnNew_Click(sender As Object, e As EventArgs)
            Response.Redirect("~/Pages/Administration/AgreementStatusEdit.aspx", False)
        End Sub

        Protected Sub gvAgreementsStatuses_RowCommand(sender As Object, e As GridViewCommandEventArgs)
            Dim id As Integer
            If Not Integer.TryParse(CStr(e.CommandArgument), id) Then Return

            Select Case e.CommandName
                Case "EditItem"
                    Response.Redirect("~/Pages/Administration/AgreementStatusEdit.aspx?id=" & id, False)

                Case "ToggleStatus"
                    Try
                        Dim current As AgreementsStatuses = _agreementStatusService.GetById(id)
                        If current Is Nothing Then
                            ShowMessage("Agreement status not found.", False)
                            Return
                        End If
                        ...
                        _agreementStatusService.ChangeStatus(id, newStatusId)
                        ShowMessage($"Agreement status '{current.Name}' updated.", True)
                        BindGrid()
        ...
        Protected Sub gvAgreementsStatuses_RowDataBound(sender As Object, e As GridViewRowEventArgs)
            If e.Row.RowType <> DataControlRowType.DataRow Then Return

            Dim agr As AgreementsStatuses = CType(e.Row.DataItem, AgreementsStatuses)
            Dim isActive As Boolean = agr.GeneralStatus IsNot Nothing AndAlso
                                      agr.GeneralStatus.Name.Equals("Active", StringComparison.OrdinalIgnoreCase)
